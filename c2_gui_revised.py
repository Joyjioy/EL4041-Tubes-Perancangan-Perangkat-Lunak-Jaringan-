...
def handleLogin():
    ...
    token = c2_logic.loggingIn(unameEntry, pwEntry, errorLabel, window)
...
def updateKategoriOptions(*args):
    ...
    for opsi in newOpt:
        catMenu["menu"].add_command(
            label=opsi,
            command=lambda value=opsi: kategoriVar.set(value)
        )
...
def handleaddTrans():
    ...
    try:
        day = int(tglVar.get())
        month = int(blnVar.get())
        year = int(thnVar.get())
        transDate = f"{year:04d}-{month:02d}-{day:02d}"
    except ValueError:
        errorAdd.config(text="Format tanggal tidak valid!", fg="#ab3730")
        return
...
def menuView():
    ...
    errorView.config(text="")
    dataTrans = c2_logic.getTrans(sesToken, errorView, window)
...
def menuRecap():
    ...
    errorRecap.config(text="")
    dataSummary = c2_logic.getSummary(sesToken, errorRecap, window)
    ...
    if dataSummary is not None:
        ...
        saldo = int(dataSummary.get("balance", 0))
...
loginBt = Button(
    ...
    bg="#b6c8df",
    fg="#3f49a5",
    activebackground="#3f49a5",
    activeforeground="#b6c8df"
)
loginBt.pack()
...
pwEntry = Entry(frameLogin, font=("Tw Cen MT", 15), width=30, show="•")
pwEntry.pack(pady=5)
...
# FRAME TAMBAH TRANSAKSI
frameAddtrans = Frame(window, bg="#FFF8EF")

titleTambah = Label(
    frameAddtrans,
    text="Penambahan Transaksi",
...
Label(
    ...
    fg="#3F49A5",
    bg="#FFF8EF"
).grid(row=3, column=0, sticky=W, pady=5, padx=10)
...
# FRAME LIHAT TRANSAKSI
frameViewtrans = Frame(window, bg="#FFF8EF")

titleView = Label(
    frameViewtrans,
    text="Daftar Transaksi Perusahaan",
    font=("Tw Cen MT", 20, "bold"),
    fg="#3F49A5",
    bg="#FFF8EF"
)
...
errorView = Label(frameViewtrans, text="", font=("Tw Cen MT", 9, "bold"), bg="#FFF8EF")
errorView.pack(pady=3)
...
for k in cols:
    ...
    tableTrans.column(
        ...
        width=columnWidth[k],
        minwidth=100,
        anchor=W if k in ["Notes", "Kategori"] else CENTER
    )

btBackfromview = Button(
    frameViewtrans,
    text="Kembali",
    command=backfromView,
    font=("Tw Cen MT", 12),
    bg="#b6c8df",
    fg="#3f49a5"
)
...
# FRAME REKAP TRANSAKSI
frameRecap = Frame(window, bg="#FFF8EF")

titleRecap = Label(
    frameRecap,
    text="Rekapitulasi Keuangan Perusahaan",
    font=("Tw Cen MT", 20, "bold"),
    fg="#3F49A5",
    bg="#FFF8EF"
)
...
errorRecap = Label(frameRecap, text="", font=("Tw Cen MT", 9, "bold"), bg="#FFF8EF")
errorRecap.pack(pady=3)
